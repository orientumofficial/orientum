...
#if defined(NDEBUG)
# error "Orientum cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9ea6ccf04f7bfcdb916e08197e158a0a998fca7a04d5783b3363000f11ba050f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Orientum: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Orientum Signed Message:\n";
...
{
    // Orientum: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Orientum: 1 Day
static const int64 nTargetSpacing = 3 * 60; // Orientum: 3 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Orientum: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Orientum: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x46ea0406a953528acb554a9c735f268eda5e2873449f86ea1aaf08c2e6f17e50");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ABC News to air exclusive interview with Thai soccer team and coach rescued from cave";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1535010929;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 135000;

        if (fTestNet)
        {
            block.nTime    = 1535010913;
            block.nNonce   = 886775;
        }        
        ...
        assert(block.hashMerkleRoot == uint256("0x55dd3a4d2009a1c20a5c1d4d7a8523ca89331574d18f09f615ce5cb630559f81"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x79, 0xd5, 0xc4, 0xf9 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OrientumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OrientumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OrientumMiner(CWallet *pwallet)
{
    printf("OrientumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Orientum-miner");
    ...
    try { loop {
        ...
        printf("Running OrientumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OrientumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OrientumMiner, pwallet));
}
